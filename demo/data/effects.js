...
const existingEffects = [
  {
    ...
    "research": {
      ...
      "difference": "Unlike the existing orthographic dot-matrix network globe, this effect streams a local 4,320 × 2,160 NOAA/NCEI ETOPO 2022 elevation texture, rasterizes Natural Earth 50m coastline topology, plots 700 named peaks across seven continents, and uses the resulting data for real vertex displacement, bathymetry, contour shading, atmospheric depth, zoom, and summit-to-summit expedition focus.",
      "verifiedAt": "2026-07-23"
    },
    "behavior": {
      "trigger": "trusted mouse/touch/pen drag, wheel zoom, arrow and zoom keys, Next summit activation, or Home/Escape reset",
      "response": "Watch real elevation, 50m coastline and 700-peak data load through an honest progress display; then orbit and zoom the displaced Earth or focus eight expedition anchors while marker, height, continent and coordinates remain spatially registered",
      "timing": "data-driven staged loading followed by direct human-owned orbit and zoom plus a finite 720 ms summit focus; no auto-rotation, autoplay, fallback, synthetic input, or preview-clock motion",
      "layer": "full-stage Three.js WebGL Earth with local ETOPO 2022 elevation and bathymetry, Natural Earth 50m land mask, 700 GPU peak markers, contour shader, atmosphere, star field, progress UI, and semantic expedition controls"
    },
    "prompt": "Implement the \"Topographic relief expedition globe\" (地形浮雕探险地球) web interaction effect in the current project.\n\nUse Three.js with a local 4,320 × 2,160 NOAA/NCEI ETOPO 2022 elevation and bathymetry texture, Natural Earth 50m land topology, and a deterministic catalog of 700 named summits. Stream the heavier relief asset with byte-aware progress, decode elevation in the shader, displace a sufficiently tessellated sphere, and shade ocean depth, alpine height, contour bands, latitude/longitude registration lines, and atmospheric rim light. The globe must be a real WebGL scene, not a flat map, CSS sphere, video, or reused dot-matrix approximation.\n\nInteraction contract:\n- Trigger: trusted pointer/touch drag, wheel zoom, arrow and +/- keys, visible summit control, or Home/Escape reset\n- Visual response: orbit and zoom the relief globe, inspect the global summit field, then focus one of eight expedition anchors while its marker, height, continent, and position stay spatially registered\n- Timing relationship: honest staged data loading followed by direct orbit and zoom plus a finite human-started focus transition; no automatic rotation or capture-clock mutation\n- Page layer: progress UI above Three.js Earth while loading, then Earth, 700 GPU peak markers and stars below accessible expedition controls and live summit evidence\n\nRequirements:\n- Show a clear loading stage, percentage, progress bar, resource detail, and useful dataset metadata; do not reveal an incomplete globe before the critical texture is ready.\n- Keep the first completed globe frame meaningful and fully static until trusted input.\n- Use deterministic local data and bundled dependencies; do not depend on remote runtime assets.\n- Preserve pointer capture, touch behavior, keyboard controls, responsive composition, prefers-reduced-motion, rendering cleanup, and context-loss-safe failure reporting.\n- Keep the displaced globe visually distinct from a dotted network globe by making real elevation, bathymetry, land shape, relief, contour bands, light, and summit focus the primary evidence.\n- Expose auditable scene state and verify the live WebGL context, exact ETOPO dimensions and byte size, 50m topology coverage, 700-peak coverage, geometry density, draw calls, loader completion, controls, and no-autoplay contract.\n\nStart from this minimal API shape:\n\n```js\nimport * as THREE from 'three';\nconst geometry = new THREE.SphereGeometry(1, 320, 192);\nconst material = new THREE.ShaderMaterial({ uniforms: { uTopology: { value: topologyTexture }, uElevationMap: { value: elevationTexture }, uTerrain: { value: 45 } }, vertexShader, fragmentShader });\nconst globe = new THREE.Mesh(geometry, material);\nscene.add(globe);\n```\n\nReturn the working code, files changed, evidence provenance, and a short explanation of how to tune streaming stages, relief exaggeration, contour density, focus duration, camera range, peak density, and reduced-motion behavior.",
    "sources": [
      {
