...
function readmeLink(locale, from) {
...
function languageNavigation(from = 'root') {
  return supportedLocales.map(locale => `[${locale.nativeName}](${readmeLink(locale, from)})`).join(' · ');
}
...
function heroBlock({ kicker, tagline, links, navigationFrom = 'root' }) {
...
<sub>${languageNavigation(navigationFrom)}</sub>

</div>`;
}

function communityGroupInvite({ label, alt, assetPrefix = '' }) {
  return `<p align="center"><strong>${escapeHtml(label)}</strong></p>

<p align="center"><img src="${assetPrefix}docs/assets/awesome-web-effects-wechat-group.png" width="420" alt="${escapeHtml(alt)}"></p>`;
}
...
function languageSupportDocument() {
  ...
  const rows = supportedLocales.map(locale => `| ${locale.rank} | ${locale.englishName} / ${locale.chineseName} | ${locale.nativeName} | ${formatStars(locale.speakersMillions)}M | [${readmeFilename(locale)}](${readmeLink(locale, 'docs')}) | [\`${locale.code}\`](${siteUrlFor(locale)}) | ${locale.dir.toUpperCase()} |`);
  return `# Supported languages / 支持语言

${languageNavigation('docs')}
...
await mkdir(resolve(root, languagesDirectory), { recursive: true });
...
const readmeWrites = supportedLocales.map(locale => {
  ...
  return writeFile(resolve(root, readmePath(locale)), content);
});

await Promise.all([
  ...readmeWrites,
  writeFile(resolve(root, 'docs', 'LANGUAGES.md'), languageSupportDocument()),
  writeFile(resolve(root, languagesDirectory, 'README.zh-CN.md'), '# 简体中文 README 已迁移\n\n[打开简体中文 README](README.zh-Hans.md)\n')
]);
