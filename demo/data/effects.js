...
const existingEffects = [
  {
    ...
    "category": "webgl",
    "name": "ETOPO topographic relief globe",
    "nameZh": "ETOPO 地形浮雕地球",
    "addedIn": "2026-effect-expansion",
    "research": {
      "sourceUrl": "https://orogen.giraffetree.cn",
      "difference": "Unlike the existing orthographic dot-matrix network globe, this effect is deliberately only the Earth itself: it streams a local 4,320 × 2,160 NOAA/NCEI ETOPO 2022 elevation texture with real byte progress, rasterizes Natural Earth 50m coastline topology, and uses those datasets for vertex displacement, bathymetry, contour shading, slow automatic rotation, orbit, and zoom without named features, annotation layers, or an outer atmosphere glow.",
      "verifiedAt": "2026-07-23"
    },
    "behavior": {
      "trigger": "resource loading starts on entry; slow automatic Earth rotation begins when ready; trusted mouse/touch/pen drag, wheel zoom, arrow and zoom keys, reset activation, or Home/Escape remain available",
      "response": "Watch real elevation and 50m coastline data load through an honest percentage bar, then observe the unannotated displaced Earth rotate automatically or orbit and zoom it directly",
      "timing": "data-driven staged loading followed by continuous 5°/s automatic rotation, paused during drag and disabled for reduced motion, plus direct orbit, zoom, and a finite 520 ms reset",
      "layer": "full-stage Three.js WebGL Earth with local ETOPO 2022 elevation and bathymetry, Natural Earth 50m land mask, contour shader, star field, synchronized progress UI, and one reset control; no atmosphere shell or outer glow"
    },
    "prompt": "Implement the \"ETOPO topographic relief globe\" (ETOPO 地形浮雕地球) web interaction effect in the current project.\n\nUse Three.js with a local 4,320 × 2,160 NOAA/NCEI ETOPO 2022 elevation and bathymetry texture plus Natural Earth 50m land topology. Stream the heavier relief asset with byte-aware progress, decode elevation in the shader, displace a sufficiently tessellated sphere, and shade ocean depth, land elevation, contour bands, latitude/longitude registration lines, and directional light. The globe must be a real WebGL scene, not a flat map, CSS sphere, video, or dot-matrix approximation.\n\nInteraction contract:\n- Trigger: resource loading begins on entry; automatic rotation begins when ready; trusted pointer/touch drag, wheel zoom, arrow and +/- keys, a visible reset control, or Home/Escape remain available\n- Visual response: rotate the unannotated relief Earth continuously at a calm pace, pause that rotation while dragging, and preserve direct orbit and zoom; reset returns to the original view before rotation resumes\n- Timing relationship: honest staged data loading followed by deterministic 5°/s automatic rotation plus a finite reset transition; disable automatic motion for prefers-reduced-motion\n- Page layer: resource percentage and progress UI above the Three.js Earth while loading, then only the relief Earth and deterministic stars below minimal controls\n\nRequirements:\n- Do not load, render, or mention named mountains, peaks, markers, labels, callouts, catalogs, or feature annotations.\n- Do not render an atmosphere shell, outer halo, glow sphere, or post-processing bloom around the planet.\n- Show a clear loading stage, percentage, progress bar, resource detail, and useful dataset metadata; synchronize the same real loader state with a same-origin detail viewer when embedded, and do not reveal an incomplete globe before the critical texture is ready.\n- Start slow automatic rotation as soon as the completed globe is revealed, pause during direct drag, resume afterward, and stop it when reduced motion is requested.\n- Use deterministic local data and bundled dependencies; do not depend on remote runtime assets.\n- Preserve pointer capture, touch behavior, keyboard controls, responsive composition, prefers-reduced-motion, rendering cleanup, and context-loss-safe failure reporting.\n- Make real elevation, bathymetry, land shape, contour bands, light, and rotation the primary evidence.\n- Expose auditable scene state and verify the live WebGL context, exact ETOPO dimensions and byte size, 50m topology coverage, zero annotation and outer-glow layers, geometry density, draw calls, loader completion, controls, and automatic-rotation contract.\n\nStart from this minimal API shape:\n\n```js\nimport * as THREE from 'three';\nconst geometry = new THREE.SphereGeometry(1, 320, 192);\nconst material = new THREE.ShaderMaterial({ uniforms: { uTopology: { value: topologyTexture }, uElevationMap: { value: elevationTexture }, uTerrain: { value: 45 } }, vertexShader, fragmentShader });\nconst globe = new THREE.Mesh(geometry, material);\nscene.add(globe);\n```\n\nReturn the working code, files changed, evidence provenance, and a short explanation of how to tune streaming stages, relief exaggeration, contour density, automatic rotation speed, camera range, reset duration, and reduced-motion behavior.",
    "sources": [
      {
        "projectId": "mrdoob-three-js",
        "recommended": true,
        "snippet": "import * as THREE from 'three';\nconst globe = new THREE.Mesh(new THREE.SphereGeometry(1, 320, 192), new THREE.ShaderMaterial({ uniforms: { uTopology: { value: topologyTexture }, uElevationMap: { value: elevationTexture }, uTerrain: { value: 45 } }, vertexShader, fragmentShader }));\nscene.add(globe);",
