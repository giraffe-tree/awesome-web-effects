...
const coreEffects = [
  // Animation engines
  effect('GSAP', 'greensock/GSAP', 'animation', 'Scroll-scrubbed master timeline', '滚动擦洗主时间线', 26600, code("import { gsap } from 'gsap';", "import { ScrollTrigger } from 'gsap/ScrollTrigger';", "gsap.registerPlugin(ScrollTrigger);", "const timeline = gsap.timeline({ scrollTrigger: { trigger: '.scene', scrub: true } });", "timeline.to('.card', { x: 80 }).to('.card', { rotate: 6 });"), { preview: 'gsap-scrolltrigger', isNew: false, localDemo: 'preview-demos/dist/scroll-scrubbed-master-timeline.html', demoSourcePath: 'preview-demos/scroll-scrubbed-master-timeline.html' }),
  effect('GSAP', 'greensock/GSAP', 'scroll', 'Pinned horizontal scroll scene', '固定式横向滚动场景', 26600, code("import { gsap } from 'gsap';", "import { ScrollTrigger } from 'gsap/ScrollTrigger';", "gsap.registerPlugin(ScrollTrigger);", "gsap.to('.track', { xPercent: -75, ease: 'none', scrollTrigger: { trigger: '.gallery', pin: true, scrub: 1, end: '+=2400' } });"), { isNew: false, localDemo: 'preview-demos/dist/pinned-horizontal-scroll-scene.html', demoSourcePath: 'preview-demos/pinned-horizontal-scroll-scene.html' }),
  effect('Motion', 'motiondivision/motion', 'animation', 'Shared-layout spring morph', '共享布局弹簧变形', 32819, code("import { animate } from 'motion';", "animate('.card', { x: 80, borderRadius: 24 }, { type: 'spring' });"), { preview: 'motion', isNew: false, localDemo: 'preview-demos/dist/shared-layout-spring-morph.html', demoSourcePath: 'preview-demos/shared-layout-spring-morph.html' }),
  effect('Anime.js', 'juliangarnier/anime', 'animation', 'Staggered transform choreography', '交错变换编排', 71056, code("import { animate, stagger } from 'animejs';", "animate('.dot', { x: 80, delay: stagger(60) });"), { localDemo: 'preview-demos/dist/staggered-transform-choreography.html', demoSourcePath: 'preview-demos/staggered-transform-choreography.html' }),
  effect('Tween.js', 'tweenjs/tween.js', 'animation', 'Render-agnostic value tween', '与渲染器无关的数值补间', 10129, code("import * as TWEEN from '@tweenjs/tween.js';", "new TWEEN.Tween({ x: 0 }).to({ x: 80 }, 600).onUpdate(({ x }) => card.style.translate = `${x}px`).start();", "requestAnimationFrame(function frame(t) { TWEEN.update(t); requestAnimationFrame(frame); });"), { localDemo: 'preview-demos/dist/render-agnostic-value-tween.html', demoSourcePath: 'preview-demos/render-agnostic-value-tween.html' }),
  effect('Mo.js', 'mojs/mojs', 'animation', 'Motion-graphics burst', '动态图形爆发', 18728, code("import mojs from '@mojs/core';", "new mojs.Burst({ parent: '#stage', radius: { 0: 80 }, count: 10 }).play();"), { localDemo: 'preview-demos/dist/motion-graphics-burst.html', demoSourcePath: 'preview-demos/motion-graphics-burst.html' }),
  effect('Theatre.js', 'theatre-js/theatre', 'animation', 'Visually authored keyframe sequence', '可视化编排关键帧序列', 12541, code("import { getProject } from '@theatre/core';", "const sheet = getProject('Demo').sheet('Scene');", "const card = sheet.object('Card', { x: 0 }); card.onValuesChange(({ x }) => element.style.translate = `${x}px`);"), { localDemo: 'preview-demos/dist/visually-authored-keyframe-sequence.html', demoSourcePath: 'preview-demos/visually-authored-keyframe-sequence.html' }),
  effect('Popmotion', 'Popmotion/popmotion', 'animation', 'Functional value pipeline', '函数式数值管线', 20167, code("import { animate } from 'popmotion';", "animate({ from: 0, to: 80, onUpdate: x => card.style.translate = `${x}px` });"), { localDemo: 'preview-demos/dist/functional-value-pipeline.html', demoSourcePath: 'preview-demos/functional-value-pipeline.html' }),
  ...
  effect('KUTE.js', 'thednp/kute.js', 'animation', 'Compact SVG shape tween', '轻量 SVG 形状补间', 2639, code("import KUTE from 'kute.js';", "KUTE.to('#shape-a', { path: '#shape-b' }, { duration: 700 }).start();"), { localDemo: 'preview-demos/dist/compact-svg-shape-tween.html', demoSourcePath: 'preview-demos/compact-svg-shape-tween.html' }),
  ...
  effect('Isotope', 'metafizzy/isotope', 'transition', 'Filterable grid reflow', '可筛选网格重排', 11103, code("import Isotope from 'isotope-layout';", "const grid = new Isotope('.grid', { itemSelector: '.item' });", "grid.arrange({ filter: '.featured' });"), { preview: 'isotope', isNew: false, legacy: true, localDemo: 'preview-demos/dist/filterable-grid-reflow.html', demoSourcePath: 'preview-demos/filterable-grid-reflow.html' }),
  ...
  effect('Floating UI', 'floating-ui/floating-ui', 'carousel', 'Anchored popover flip and shift', '锚点浮层翻转与位移', 32665, code("import { computePosition, flip, shift } from '@floating-ui/dom';", "computePosition(button, tooltip, { middleware: [flip(), shift()] }).then(({ x, y }) => Object.assign(tooltip.style, { left: `${x}px`, top: `${y}px` }));"), { localDemo: 'preview-demos/dist/anchored-popover-flip-and-shift.html', demoSourcePath: 'preview-demos/anchored-popover-flip-and-shift.html' }),
  ...
  effect('vanilla-tilt.js', 'micku7zu/vanilla-tilt.js', 'pointer', 'Perspective tilt and glare', '透视倾斜与高光', 4019, code("import VanillaTilt from 'vanilla-tilt';", "VanillaTilt.init(document.querySelector('.card'), { max: 12, glare: true });"), { preview: 'vanilla-tilt', isNew: false, localDemo: 'preview-demos/dist/perspective-tilt-and-glare.html', demoSourcePath: 'preview-demos/perspective-tilt-and-glare.html' }),
  ...
  // Text and SVG
  effect('Vivus', 'maxwellito/vivus', 'vector', 'SVG stroke drawing', 'SVG 描边绘制', 15479, code("import Vivus from 'vivus';", "new Vivus('logo', { duration: 120, type: 'oneByOne' });"), { preview: 'vivus', isNew: false, legacy: true, localDemo: 'preview-demos/dist/svg-stroke-drawing.html', demoSourcePath: 'preview-demos/svg-stroke-drawing.html' }),
  ...
  effect('p5.js', 'processing/p5.js', 'canvas', 'Sketch-style creative coding loop', '草图式创意编程循环', 23797, code("import p5 from 'p5';", "new p5(p => { p.setup = () => p.createCanvas(320, 180); p.draw = () => p.circle(p.mouseX, p.mouseY, 40); });"), { localDemo: 'preview-demos/dist/sketch-style-creative-coding-loop.html', demoSourcePath: 'preview-demos/sketch-style-creative-coding-loop.html' }),
