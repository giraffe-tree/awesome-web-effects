...
1. **先通过完整交互资格门**：演示中必须能辨认出 `输入 → 用户意图 → 系统反馈` 的闭环。只有模型、协议、识别叠层或数据管线而没有第一方可运行体验时，不单独收录。
2. **再按交互方式去重**：用“输入模态 + 用户意图 + 反馈位置 + 时间关系 + 协作拓扑”作为去重签名；五项基本相同才视为同一种方式。
3. **重复时优先 star 最高者**：通过资格门的同类候选中，主列表只保留 stars 最高的代表项目。底层库的高 stars 不能挤掉展示完整闭环的原型。
4. **主列表门槛为 100 stars**：低于 100 stars 但原型可运行、交互思路独特且有清晰演示的项目进入 [Backup](#backup)。
5. **必须能看见交互**：没有公开演示视频、动态图或可操作 demo，无法判断实际反馈循环的项目暂不收录。主仓库与同组织官方 samples 可共同构成证据。
6. **stars 是快照**：数量会随时间变化，README 中记录核验日期，不作为质量的永久结论。
...
### Cross-device Co-editing · 跨设备实时共编

#### [tldraw](https://github.com/tldraw/tldraw) · ★ 48,772 · 9/10

同一画布实时同步到桌面、手机和多人会话，参与者能看见远端光标、对象变化与共同结果。这里收录的重点是“远端操作立即出现在共享空间”的协作反馈，而不是与 Excalidraw 重复比较无限画布功能。

![tldraw cross-device co-editing demo](demo/gifs/tldraw.gif)
...
### Programmable Document · 可编程文档

#### [JupyterLab](https://github.com/jupyterlab/jupyterlab) · ★ 15,230 · 9/10

把叙述、代码、控件和运行结果放在同一份文档中；执行单元后，反馈原地成为文档状态的一部分。用户可以阅读、修改、运行和解释同一对象，不必在编辑器、终端与报告之间来回切换。

![JupyterLab programmable document demo](demo/gifs/jupyterlab.gif)

*Stars 核验：2026-07-15 · [原始演示素材](https://github.com/jupyterlab/jupyterlab/blob/main/docs/source/user/images/toc/runcell.gif)*

### One-to-many Device Control · 一对多设备控制

#### [QtScrcpy](https://github.com/barry-ran/QtScrcpy) · ★ 30,509 · 9/10

一个桌面输入可以同时广播到多台 Android 设备，并排显示每台设备的实时反馈。交互拓扑从“一人—一设备”扩展成“一人—多设备”，适合批量演示、测试和设备群操作。

![QtScrcpy one-to-many device control demo](demo/gifs/qtscrcpy.gif)

*Stars 核验：2026-07-15 · [原始演示素材](https://github.com/barry-ran/QtScrcpy/blob/dev/docs/image/group-control.gif)*

### Fuzzy Command Palette · 模糊命令面板

#### [fzf](https://github.com/junegunn/fzf) · ★ 81,737 · 10/10

用户只输入记得住的少量字符，候选列表持续缩小并即时预览，确认后直接执行或打开结果。它把“先找到准确路径再行动”压缩成“回忆片段—筛选—行动”的单一键盘循环。

![fzf fuzzy command palette demo](demo/gifs/fzf.gif)

*Stars 核验：2026-07-15 · [原始演示素材](https://github.com/junegunn/i/blob/master/fzf.gif)*

## Backup

低于 100 stars、但交互思路可行且有演示证据的实验性项目会放在这里。

### Haptic Timeline Authoring · 触觉时间线创作

#### [Haptics Studio](https://github.com/facebook/haptics-studio) · ★ 46 · 5/10 · Backup

在可视时间线上编辑触觉强度包络、强调点和音频转触觉结果，再通过设备预览身体反馈。它形成“视觉编排—触觉体验—继续调整”的跨模态创作循环；由于尚未达到 100 stars，按规则进入 Backup。

![Haptics Studio tactile authoring demo](demo/gifs/haptics-studio.gif)

*Stars 核验：2026-07-15 · [原始演示素材](https://github.com/facebook/haptics-studio/blob/main/frontend/src/images/intro.mp4)*
