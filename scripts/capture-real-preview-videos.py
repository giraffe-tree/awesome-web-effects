#!/usr/bin/env python3
"""Capture deterministic H.264 preview videos and static WebP posters from real demos."""
...
VIDEO_OUTPUT_ROOT = ROOT / "demo" / "videos" / "captured"
POSTER_OUTPUT_ROOT = ROOT / "demo" / "videos" / "posters"
DEFAULT_WIDTH = 320
DEFAULT_HEIGHT = 180
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--capture-scale", type=float, default=2, help="Output scale applied without changing the demo viewport")
    parser.add_argument("--playback-fps", type=int, default=6, help="Base playback cadence before intentional frame holds")
    parser.add_argument("--video-fps", type=int, default=30, help="Constant frame rate of the encoded H.264 video")
    parser.add_argument("--video-crf", type=int, default=23, help="H.264 constant-rate-factor quality (lower is higher quality)")
    parser.add_argument("--poster-quality", type=int, default=82, help="Static WebP poster quality (0-100)")
    ...
    parser.add_argument("--keep-frames", action="store_true", help="Keep captured PNG frames under tmp/real-preview-video-frames")
...
def encode_mp4(
    ...
    if not frame_delays:
        raise RuntimeError("Cannot encode an MP4 without captured frames")
    output.parent.mkdir(parents=True, exist_ok=True)
    concat_path = frame_root / "frames.ffconcat"
    concat_lines = ["ffconcat version 1.0"]
    for index, delay in enumerate(frame_delays):
        concat_lines.extend([
            f"file '{index:04d}.png'",
            f"duration {delay:.9f}",
        ])
    # The concat demuxer applies the last duration only when the final frame is
    # repeated. This duplicate is folded into the CFR output by the fps filter.
    concat_lines.append(f"file '{len(frame_delays) - 1:04d}.png'")
    concat_path.write_text("\n".join(concat_lines) + "\n")
    expected_duration = sum(frame_delays)
    ...
    subprocess.run(
        [
            ...
            "-an",
            "-vf", f"fps={fps},scale=trunc(iw/2)*2:trunc(ih/2)*2:flags=lanczos,format=yuv420p",
            "-c:v", "libx264",
            "-preset", "slow",
            "-crf", str(crf),
            "-pix_fmt", "yuv420p",
    ...
    if output.stat().st_size < 4096:
        raise RuntimeError(f"Encoded MP4 is unexpectedly small: {output}")

    probe = json.loads(subprocess.check_output(
        [
            require_command("ffprobe"), "-v", "error",
    ...
    if len(video_streams) != 1 or audio_streams:
        raise RuntimeError(f"Encoded MP4 must contain one video stream and no audio: {probe!r}")
    stream = video_streams[0]
    if stream.get("codec_name") != "h264" or stream.get("pix_fmt") != "yuv420p":
        raise RuntimeError(f"Encoded MP4 must use H.264/yuv420p: {stream!r}")
    if int(stream.get("width", 0)) % 2 or int(stream.get("height", 0)) % 2:
        raise RuntimeError(f"Encoded MP4 dimensions must be even: {stream!r}")
    encoded_duration = float(probe.get("format", {}).get("duration", 0))
    if abs(encoded_duration - expected_duration) > max(.15, 2 / fps):
        raise RuntimeError(
            f"Encoded MP4 duration {encoded_duration:.3f}s differs from requested "
            f"{expected_duration:.3f}s"
        )
    atom_prefix = output.read_bytes()[:1_048_576]
    moov_index = atom_prefix.find(b"moov")
    mdat_index = atom_prefix.find(b"mdat")
    if moov_index < 0 or mdat_index < 0 or moov_index > mdat_index:
        raise RuntimeError(f"Encoded MP4 is not fast-start optimized: {output}")


def encode_poster(frame_root: Path, output: Path, quality: int) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    try:
        from PIL import Image, features
    except ImportError as error:
        raise RuntimeError("Pillow with WebP support is required: pip install Pillow") from error
    if not features.check("webp"):
        raise RuntimeError("The installed Pillow build does not support WebP")
    with Image.open(frame_root / "0000.png") as captured:
        frame = captured.convert("RGB")
        frame.save(output, format="WEBP", quality=quality, method=6)
        frame.close()
    if output.stat().st_size < 4096:
        raise RuntimeError(f"Encoded WebP poster is unexpectedly small: {output}")
...
def capture_demo(page, url: str, demo: dict, video_frame_root: Path, args: argparse.Namespace) -> tuple[int, int, list[float]]:
    errors: list[str] = []
    ...
    base_delay = 1 / args.playback_fps
    ...
    for index in range(frame_count):
        ...
        frame_path = video_frame_root / f"{index:04d}.png"
        frame_capture = cdp_session.send("Page.captureScreenshot", {
            "format": "png",
            "fromSurface": True,
            "captureBeyondViewport": False,
            "clip": {"x": 0, "y": 0, "width": args.width, "height": args.height, "scale": args.capture_scale},
        })
        frame_bytes = base64.b64decode(frame_capture["data"])
        frame_path.write_bytes(frame_bytes)
        if pending_hold:
            frame_delays[index] = max(frame_delays[index], pending_hold)
            pending_hold = 0.0
        hashes.add(hashlib.sha256(frame_bytes).hexdigest())
...
def main() -> int:
    ...
    if (
        ...
        or args.width < 64
        or args.height < 64
        or args.capture_scale < 1
    ):
        raise RuntimeError(
            "fps, playback-fps, video-fps, duration, width, height, and capture-scale "
            "must be positive capture values"
        )
    scaled_width = args.width * args.capture_scale
    scaled_height = args.height * args.capture_scale
    if (
        not float(scaled_width).is_integer()
        or not float(scaled_height).is_integer()
        or int(scaled_width) % 2
        or int(scaled_height) % 2
    ):
        raise RuntimeError("width and height multiplied by capture-scale must be even integers")
    if not 0 <= args.video_crf <= 51:
        raise RuntimeError("video-crf must be between 0 and 51")
    if not 0 <= args.poster_quality <= 100:
        raise RuntimeError("poster-quality must be between 0 and 100")
    ...
    shard_label = f"shard-{args.shard.replace('/', '-of-')}" if args.shard else "all"
    run_label = f"{shard_label}-{os.getpid()}"
    vite_log = log_root / f"real-preview-vite-{run_label}.log"
    ...
    try:
        ...
        POSTER_OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
        persistent_frames = ROOT / "tmp" / "real-preview-video-frames" / run_label if args.keep_frames else None
            ...
            for demo in demos:
                ...
                try:
                    if persistent_frames:
                        ...
                        frame_count, unique_count, frame_delays = capture_demo(
                            page, demo_url(demo), demo, frame_root, args
                        )
                        encode_mp4(
                            frame_root,
                            VIDEO_OUTPUT_ROOT / f"{demo['id']}.mp4",
                            frame_delays,
                            args.video_fps,
                            args.video_crf,
                        )
                        encode_poster(
                            frame_root,
                            POSTER_OUTPUT_ROOT / f"{demo['id']}.webp",
                            args.poster_quality,
                        )
                    else:
                    ...
                    page.close()
                video_size_kib = (VIDEO_OUTPUT_ROOT / f"{demo['id']}.mp4").stat().st_size / 1024
                poster_size_kib = (POSTER_OUTPUT_ROOT / f"{demo['id']}.webp").stat().st_size / 1024
                loop_seconds = sum(frame_delays)
                print(
                    f"Captured {demo['id']}: {frame_count} frames, {unique_count} unique, "
                    f"MP4 {loop_seconds:.1f}s/{video_size_kib:.1f} KiB, "
                    f"poster {poster_size_kib:.1f} KiB",
                    flush=True,
                )
                success_count += 1
    ...
    print(f"Wrote {success_count} H.264 preview video(s) to {VIDEO_OUTPUT_ROOT}")
    print(f"Wrote {success_count} static WebP poster(s) to {POSTER_OUTPUT_ROOT}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"capture-real-preview-videos: {error}", file=sys.stderr)
        raise SystemExit(1)
