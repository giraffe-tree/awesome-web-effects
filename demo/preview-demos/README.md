...
These pages are the capture sources for six WebGL catalog previews. Each page imports and runs the library named by the catalog entry; none is a hand-drawn approximation or a shared placeholder scene.
...
From the repository root, build the static pages and capture all six deterministic GIFs:
...
Use `--only=effect-id` to capture one page. Add `--built` to capture and verify the committed static `dist` pages exactly as GitHub Pages will serve them. The script checks the page-reported library metadata, a live WebGL context, readiness, and frame variation before it writes `demo/gifs/captured/<effect-id>.gif`.
