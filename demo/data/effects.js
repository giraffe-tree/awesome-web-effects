...
const existingEffects = [
  {
    ...
    "research": {
      ...
      "difference": "Unlike the existing orthographic dot-matrix network globe, this effect is deliberately only the Earth itself: it streams a local 4,320 × 2,160 NOAA/NCEI ETOPO 2022 elevation texture, rasterizes Natural Earth 50m coastline topology, and uses those two datasets for vertex displacement, bathymetry, contour shading, atmospheric depth, orbit, and zoom without named features or annotation layers.",
      "verifiedAt": "2026-07-23"
    },
    "behavior": {
      "trigger": "trusted mouse/touch/pen drag, wheel zoom, arrow and zoom keys, reset activation, or Home/Escape",
      "response": "Watch real elevation and 50m coastline data load through an honest progress display, then orbit or zoom the unannotated displaced Earth",
      "timing": "data-driven staged loading followed by direct human-owned orbit and zoom plus a finite 520 ms reset; no auto-rotation, autoplay, fallback, synthetic input, or preview-clock motion",
      "layer": "full-stage Three.js WebGL Earth with local ETOPO 2022 elevation and bathymetry, Natural Earth 50m land mask, contour shader, atmosphere, star field, progress UI, and one reset control"
    },
    "prompt": "Implement the \"ETOPO topographic relief globe\" (ETOPO 地形浮雕地球) web interaction effect in the current project.\n\nUse Three.js with a local 4,320 × 2,160 NOAA/NCEI ETOPO 2022 elevation and bathymetry texture plus Natural Earth 50m land topology. Stream the heavier relief asset with byte-aware progress, decode elevation in the shader, displace a sufficiently tessellated sphere, and shade ocean depth, land elevation, contour bands, latitude/longitude registration lines, and atmospheric rim light. The globe must be a real WebGL scene, not a flat map, CSS sphere, video, or dot-matrix approximation.\n\nInteraction contract:\n- Trigger: trusted pointer/touch drag, wheel zoom, arrow and +/- keys, a visible reset control, or Home/Escape\n- Visual response: orbit and zoom the unannotated relief Earth; reset returns to the original view\n- Timing relationship: honest staged data loading followed by direct human-owned orbit and zoom plus a finite reset transition; no automatic rotation or capture-clock mutation\n- Page layer: progress UI above the Three.js Earth while loading, then only the relief Earth, atmosphere and deterministic stars below minimal controls\n\nRequirements:\n- Do not load, render, or mention named mountains, peaks, markers, labels, callouts, catalogs, or feature annotations.\n- Show a clear loading stage, percentage, progress bar, resource detail, and useful dataset metadata; do not reveal an incomplete globe before the critical texture is ready.\n- Keep the first completed globe frame meaningful and fully static until trusted input.\n- Use deterministic local data and bundled dependencies; do not depend on remote runtime assets.\n- Preserve pointer capture, touch behavior, keyboard controls, responsive composition, prefers-reduced-motion, rendering cleanup, and context-loss-safe failure reporting.\n- Make real elevation, bathymetry, land shape, contour bands, light, and atmosphere the primary evidence.\n- Expose auditable scene state and verify the live WebGL context, exact ETOPO dimensions and byte size, 50m topology coverage, zero annotation layers, geometry density, draw calls, loader completion, controls, and no-autoplay contract.\n\nStart from this minimal API shape:\n\n```js\nimport * as THREE from 'three';\nconst geometry = new THREE.SphereGeometry(1, 320, 192);\nconst material = new THREE.ShaderMaterial({ uniforms: { uTopology: { value: topologyTexture }, uElevationMap: { value: elevationTexture }, uTerrain: { value: 45 } }, vertexShader, fragmentShader });\nconst globe = new THREE.Mesh(geometry, material);\nscene.add(globe);\n```\n\nReturn the working code, files changed, evidence provenance, and a short explanation of how to tune streaming stages, relief exaggeration, contour density, camera range, reset duration, and reduced-motion behavior.",
    "sources": [
      {
